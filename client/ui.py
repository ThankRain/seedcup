...
class UI(object):
    ...
    def display(self):
        ...
        print(
            "playerID: {self.playerID}, color: {color2emoji[self.color].emoji()}, characterNum: {len(self.characters)}, character: {playerID2emoji[self.playerID].emoji()}, score: {self.score}, killNum: {self.kill}"
        )

        for character in self._characters:
            print("characterState: {character}")
        print("\n")
        for x in range(self.mapSize):
